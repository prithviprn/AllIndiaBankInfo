package com.ashoksm.allindiabankinfo.sqlite;
...
import com.ashoksm.allindiabankinfo.DisplayBankBranchResultActivity;
        ...
        try {
            ...
            for (String name : fileNames) {
                if (name.endsWith(".sql") && name.startsWith("banklocation")) {
                    ...
                    context.runOnUiThread(new Runnable() {
                        public void run() {
                            mProgressDialog.setProgress(mProgressDialog.getProgress() + 3);
                        }
                    });
                }
            }
